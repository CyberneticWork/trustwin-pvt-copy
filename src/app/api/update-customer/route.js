...
export async function POST(req) {
  ...
  try {
    ...
    const {
      prefix, fullname, nic, gender, dob, location, address, telno, gs, ds, district, province, status, createby, editby
    } = data;
    console.log('Main fields:', { prefix, fullname, nic, gender, dob, location, address, telno, gs, ds, district, province, status, createby, editby });

    // Validate required fields
    if (!prefix || !fullname || !nic || gender === undefined || !dob || !location || !address || !telno || !gs || !ds || !district || !province || !status) {
      console.log('Validation failed: missing required fields');
      return Response.json({ error: "Missing required fields", code: "INVALID_INPUT" }, { status: 400 });
    }
    // Validate NIC format
    if (!(/^\d{12}$/.test(nic) || /^\d{9}[VvXx]$/.test(nic))) {
      console.log('Validation failed: invalid NIC format');
      return Response.json({ error: "Invalid NIC format", code: "INVALID_NIC" }, { status: 400 });
    }
    // Validate gender
    if (!(gender === 0 || gender === 1)) {
      console.log('Validation failed: invalid gender');
      return Response.json({ error: "Invalid gender value", code: "INVALID_GENDER" }, { status: 400 });
    }
    // Validate DOB
    if (isNaN(Date.parse(dob))) {
      console.log('Validation failed: invalid DOB');
      return Response.json({ error: "Invalid date of birth", code: "INVALID_DOB" }, { status: 400 });
    }
    ...
    const [existingRows] = await connection.execute(
      'SELECT status FROM update_customer WHERE nic = ?', [nic]
    );
    ...
    if (existingRows.length > 0) {
      ...
      await connection.execute(
        ...
        [prefix, fullname, gender, dob, location, address, telno, gs, ds, district, province, status, editby || 'system', nic]
      );
      if (data.spouse) {
        ...
        const [spouseRows] = await connection.execute(
          'SELECT id FROM update_spouse WHERE nic = ?', [nic]
        );
        ...
        if (spouseRows.length > 0) {
          ...
          await connection.execute(
            ...
            [nic, name, address, telno, relation, 'N/A', createby || 'system', editby || 'system']
          );
        }
      }
      await connection.end();
      console.log('DB connection closed (update path)');
      return Response.json({ message: "Request updated", code: "SUCCESS" }, { status: 200 });
    } else {
      ...
      await connection.execute(
        `INSERT INTO update_customer (prefix, fullname, nic, gender, dob, location, address, telno, gs, ds, district, province, status, createby, editby)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [prefix, fullname, nic, gender, dob, location, address, telno, gs, ds, district, province, status, createby || 'system', editby || 'system']
      );
      if (data.spouse) {
        const { name, address, telno, relation } = data.spouse;
        console.log('Spouse payload:', data.spouse);
      ...
      return Response.json({ message: "Request created", code: "SUCCESS" }, { status: 201 });
    }
  } catch (error) {
    ...
    } else if (code === "INVALID_INPUT" || code === "INVALID_NIC" || code === "INVALID_GENDER" || code === "INVALID_DOB") {
      status = 400;
    }
