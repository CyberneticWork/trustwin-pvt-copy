  ...
  try {
    ...
    try {
      ...
      const [equipmentStats] = await connection.execute(`
        SELECT 
          COUNT(*) as total,
          SUM(CASE WHEN status = 'Waiting for Funds' THEN 1 ELSE 0 END) as waiting,
          SUM(CASE WHEN status = 'active' THEN 1 ELSE 0 END) as approved,
          SUM(CASE WHEN status = 'rejected' THEN 1 ELSE 0 END) as rejected
        FROM equipment_loan_applications
        WHERE status IN ('Waiting for Funds', 'active', 'rejected')
      `);

      // Calculate combined stats
      const totalLoans =
        Number(autoStats[0].total) +
        Number(businessStats[0].total) +
        Number(equipmentStats[0].total);
      const waitingLoans =
        Number(autoStats[0].waiting) +
        Number(businessStats[0].waiting) +
        Number(equipmentStats[0].waiting);
      const approvedLoans =
        Number(autoStats[0].approved) +
        Number(businessStats[0].approved) +
        Number(equipmentStats[0].approved);
      const rejectedLoans =
        Number(autoStats[0].rejected) +
        Number(businessStats[0].rejected) +
        Number(equipmentStats[0].rejected);
