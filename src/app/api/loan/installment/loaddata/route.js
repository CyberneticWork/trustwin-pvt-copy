  ...
  try {
    ...
    if (loanType === "Hire Purchase") {
      ...
      loanDetails = {
        ...
        updatedAt: hirePurchaseLoan.updated_at
          ? formatDate(hirePurchaseLoan.updated_at)
          : null,
      };
      ...
      try {
        if (
          ...
          const match = String(businessLoan.term).match(
            /(\d+)\s*(day|month|week|year)s?/i
          );
          if (match) {
          ...
          for (let i = 0; i < nPeriods; i++) {
            ...
            if (periodType === "month")
              ...
              dueDate.setDate(dueDate.getDate() + periodValue - 1); // single payment at end
            scheduleArr.push({
              payment_number: i + 1,
              payment_date: `${dueDate.getFullYear()}-${String(
                dueDate.getMonth() + 1
      ...
      loanDetails = {
        ...
        loanAmount: businessLoan.loanAmount
          ? parseFloat(businessLoan.loanAmount).toFixed(2)
          : null,
        loanTermMonths: null,
        interestRate:
          businessLoan.rate !== undefined
            ? parseFloat(businessLoan.rate).toFixed(2)
            : null,
        monthlyPayment:
          businessLoan.Installment !== undefined
            ? parseFloat(businessLoan.Installment).toFixed(2)
            : null,
        totalInterest: null,
        totalPayable:
          businessLoan.Totalpay !== undefined
            ? parseFloat(businessLoan.Totalpay).toFixed(2)
            : null,
        effectiveRate: null,
        clientReceivingAmount:
          businessLoan.IssueAmmount !== undefined
            ? parseFloat(businessLoan.IssueAmmount).toFixed(2)
            : null,
        ...
        updatedAt: null,
        initialPay:
          businessLoan.initialPay !== undefined
            ? parseFloat(businessLoan.initialPay).toFixed(2)
            : null,
      ...
      return new Response(
        JSON.stringify({
          code: "ERROR",
          message:
            "Invalid loanType. Must be 'Hire Purchase' or 'Business Loan'",
        }),
        { status: 400 }
      );
    }
    ...
    if (paymentRows.length > 0) {
      lastPaymentDate = paymentRows[0].payment_date
        ? formatDate(paymentRows[0].payment_date)
        : "";
      lastPaidAmount = paymentRows[0].amount_paid
        ? paymentRows[0].amount_paid.toString()
        : "";
      paidRentalAmount = paymentRows
        .reduce((sum, row) => sum + parseFloat(row.amount_paid), 0)
        .toString();
    ...
    try {
      ...
      let schedule = [];
      let periodCount = 0;
      let scheduleStart = rawContractStartDate
        ? new Date(rawContractStartDate)
        : null;
      ...
      let payments = Array.isArray(paymentRows)
        ? paymentRows
            .slice()
      ...
      let paymentIdx = 0;
      let paymentLeft =
        payments.length > 0 ? parseFloat(payments[0].amount_paid) : 0;
        ...
        while (paymentIdx < payments.length && due.paid < due.amountDue) {
          if (paymentLeft <= 0) {
            ...
            if (paymentIdx < payments.length)
              paymentLeft = parseFloat(payments[paymentIdx].amount_paid);
            else break;
          }
      ...
      advancedArrears = schedule.reduce(
        (sum, due) => sum + Math.max(0, due.amountDue - due.paid),
        0
      );
